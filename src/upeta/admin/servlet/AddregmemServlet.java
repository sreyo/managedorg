...
/**
 * Servlet implementation class AddregmemServlet
 */
@WebServlet("/AddregmemServlet")
public class AddregmemServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AddregmemServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String[] mal=request.getParameterValues("mal[]");
        String[] hin=request.getParameterValues("hin[]");
        ...
        for (int i=0;i<id;i++) { // stmt = con.createStatement(); fname1 =
		     ...
		     try {
				status=Memberdao.insertregMyEntity(hostname,sesid, fname[i], lname[i], email[i], phone[i], relat[i], dob[i], gender[i], address[i],address1[i], city[i], state[i], country[i], zip[i],khometown[i],monthdob[i],password[i],mal[i],hin[i]);
